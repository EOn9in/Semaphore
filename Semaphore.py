...
# Сигналы для различных поз пальцев
SEMAPHORES = {
  (-90, -45): {'a': "a", 'n': "1"},
  (-90, 0): {'a': "b", 'n': "2"},
  (-90, 45): {'a': "c", 'n': "3"},
  ...
  (180, 45): {'a': "y"},
  (180, 225): {'a': "z"}, # Стереть последний сигнал
  (90, 90): {'a': "space", 'n': "enter"},
  (135, 90): {'a': "tab"}, # Замена пользовательских "цифровых" сигналов
  (225, 45): {'a': "escape"}, # Замена пользовательских сигналов "отмена"
}
# Углы стрелок ног
leg_arrow_angles = {
...
FRAME_HISTORY = 8 # История позы сравнивается с LAST_FRAMES последними кадрами
HALF_HISTORY = int(FRAME_HISTORY/2)
QUARTER_HISTORY = int(FRAME_HISTORY/4)

empty_frame = {  # Пустой кадр для инициализации
    'hipL_y': 0,  # Координата y левого бедра
    'hipR_y': 0,  # Координата y правого бедра
    'hips_dy': 0,  # Расстояние между бедрами по вертикали
    'dxL_thrust_hipL': 0,  # Расстояние между левым бедром и тазом (горизонтальное)
    'dxL_thrust_hipR': 0,  # Расстояние между правым бедром и тазом (горизонтальное)
    'dxR_thrust_hipL': 0,  # Расстояние между левым бедром и тазом (горизонтальное)
    'dxR_thrust_hipR': 0,  # Расстояние между правым бедром и тазом (горизонтальное)
    'signed': False,  # Признак
}
last_frames = FRAME_HISTORY*[empty_frame.copy()]  # История рамки

frame_midpoint = (0, 0)  # Средняя точка рамки

current_semaphore = ''  # Текущий сигнал
last_keys = []  # Последние клавиши
